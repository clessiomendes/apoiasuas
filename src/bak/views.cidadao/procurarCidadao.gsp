...
<div id="list-cidadao" class="content scaffold-list" role="main">
    <table>
        ...
        <tbody>
        <g:each in="${cidadaoInstanceList}" status="i" var="cidadaoInstance">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">

                <td><g:link action="selecionarFamilia" id="${cidadaoInstance.familia.id}">
                    ${fieldValue(bean: cidadaoInstance, field: "familia.codigoLegado")}
                </g:link></td>

%{--
                <td><g:link controller="familia" action="show" id="${cidadaoInstance.familia.codigoLegado}">
                    ${fieldValue(bean: cidadaoInstance, field: "familia.codigoLegado")}
                </g:link></td>
--}%

                <td>${fieldValue(bean: cidadaoInstance, field: "nomeCompleto")}</td>

...
class DeployOptionsProducao extends DeployOptionsBaseClass {
    static {
        permSize = "200m"
        heapSize = "750m"
        timeZone = "America/Sao_Paulo"
...
target(deploy: "Deploy para todos os ambientes") {
    depends(war)
    def deployOptions
    if (Environment.current == Environment.PRODUCTION) {
        System.err.println "********* ATENÇÃO!!!!!! Deploy em PRODUÇÃO ***********************************************************"
        System.out.println "********* ATENÇÃO!!!!!! Deploy em PRODUÇÃO ***********************************************************"
        deployOptions = new DeployOptionsProducao()
    } else {
        System.out.println "Deploy em ${Environment.current}"
        deployOptions = new DeployOptionsValidacao()
    }
    executa(deployOptions)
}

//target(deployProducao: "Deploy para ambiente de PRODUÇÃO") {
//    depends(war)
//    def deployOptions = new DeployOptionsProducao()
//    executa(deployOptions)
//}
